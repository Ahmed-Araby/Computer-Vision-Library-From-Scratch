...
/*
 [Q]
 why parameters are void pointers
 and why casting work well here !?
*/
...
{// this combare two patches (descriptors)
    // TODO: return the correct number.
    // n supposed to be the min between a and b
    int x=0;
    float sum=0;
    for(x=0; x<n; x++)
    {
        float v1=a[x];
        float v2=b[x];
        sum+=fabs(v1-v2);   // abs for int , fabs for float
    }
...
match *match_descriptors(descriptor *a, int an, descriptor *b, int bn, int *mn)
{
    ...
    // We will have at most an matches. ,   why not bn !!??? , after all we will endup with fmin(an , bn)  matches
    *mn = an;
    ...
    for(j = 0; j < an; ++j){
        ...
        for(x=0; x<bn; x++)
        {
            float tmp=l1_distance(a[j].data , b[x].data , fmin(a[j].n , b[x].n));  // minf !!! *******
    ...
    int count = 0;
    int *seen = calloc(bn, sizeof(int)); // mark on bn describtor as taken (vis array) , calloc initialize with 0
    ...
    for(x=0; x<an; x++)
    {
        ...
        if(seen[bi]==1)
            continue;
        m[count]=m[x];  // bring it in front or leave it as it's
        seen[bi]=1;
        count++;
    }
    *mn = count;  // count is the actual size of good matches  , all remanning are repeated matches
    free(seen);
    return m;
}
